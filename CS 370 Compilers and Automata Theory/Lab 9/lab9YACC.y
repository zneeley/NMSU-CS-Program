...
%token <value> NUM
%token <string> ID INT VOID WHILE DO IF THEN ELSE READ WRITE FOR RETURN LE GE EQ NE LT GT STRING
%token AND OR NOT 
%token MYBEGIN MYEND OF BOOLEAN MYTRUE MYFALSE 
...
program     : declList {myprogram = $1; /* ASToutput(0, $$);  Pass the generated tree to program */} 
            ;
...
funDecl     : typeSpec ID { struct SymbTab *p; if (mydebug) Display();
...
factor      : '(' expression ')' {$$ = $2; /* Pass the generated tree to factor */}
            ...
            | MYTRUE { $$ = ASTCreateNode(NUMBER);
                     $$->value = 1;
                     $$->isType = BOOLTYPE;
					 /* Create a node of NUMBER setting its value to 1 */
                   } 
            | MYFALSE { $$ = ASTCreateNode(NUMBER);
                      $$->value = 0;
                      // Set the type
                      $$->isType = BOOLTYPE;
