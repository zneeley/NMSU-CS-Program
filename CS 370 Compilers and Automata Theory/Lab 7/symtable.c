...
// This function will compare the incoming params and args to each to see if they match.
int compareFormula(ASTnode *fparms, ASTnode *args) {

    // If they both end up being NULL just say they are the same.
    if ((fparms == NULL) && (args == NULL)) {
        return 1;
    }
    
    // If fparms is NULL show they are not the same
    if ((fparms == NULL)) {
        return 0;
    }
    
    // If args is NULL show they are not the same
    if ((args == NULL)) {
        return 0;
    }
    
    // If they are the same recursively check to see if the next is the same
